...
class Gallery extends Controller {
  ...
  public function photos_new($album_id) {
      ...
      $album_name = $this->gallery->albums_name($album_id);
  ...
  public function photos_add($album_id) {
    ...
    try {
       ...
       header("Location: /index.php/gallery/albums_show/$photo_name");
    } catch (Exception $e) {
  ...
  public function photos_delete($album_id, $photo_id) {
    ...
    $this->gallery->delete_photo($album_id, $photo_id);
    $this->loader->load('albums_show', ['title'=>$album_id,
                                        'album'=>$album_id,
                                        'photos'=>$this->gallery->photos($album_id)
                                        ]);
  ...
  public function photos_show($album_id,$photo_id) {
    try {
      $album_id = filter_var($album_id);
      $album_id = filter_var($photo_id);
      $title = "$album_id/$photo_id";
